defmodule ExChess.Core.Pieces.Pawn do
  ...
  def pawn_regular(board, [y, _] = location, %{user: %{color: "black"}}, _acc) when y <= 7 do
    {[location_y, location_x], _occupant} =
  ...
  def pawn_regular(board, [y, _] = location, %{user: %{color: "white"}}, _acc) when y >= 2 do
    {[location_y, location_x], %{color: "white"}} =
      case Chessboard.get_location(board, location) do
          {_, %{color: "black"}} = piece ->
            piece
